...
class db():
    ...
    with codecs.open(r'C:\Users\shiri\Desktop\a.ini', 'r') as f:
        config.read_file(f)
    ...
    connect = Elasticsearch(cloud_id=conection["cloud_id"], http_auth=(conection["usr"],conection["password"]), verify_certs=conection["verify_certs"])
...
#,repository=get_repository["repository"], local=get_repository["local"], master_timeout=get_repository["master_timeout"]
    def snapshot_get_repository(self):
        try:
            x = self.connect.snapshot.get_repository(repository=self.get_repository["repository"],local=self.get_repository["local"], master_timeout=self.get_repository["master_timeout"])
...
#,repository=cleanup_repository["repository"],master_timeout=cleanup_repository["master_timeout"],timeout=cleanup_repository["timeout"]
    def snapshot_clean_repository(self):
        try:
            x = self.connect.snapshot.cleanup_repository(repository=self.cleanup_repository["repository"],master_timeout=self.cleanup_repository["master_timeout"],timeout=self.cleanup_repository["timeout"])
        except Exception as e:
            print(e)
        return x
...
#,repository=create_repository["repository"],snapshot=create_repository["body"],body=create_repository["master_timeout"],master_timeout=create_repository["master_timeout"]
    def snapshot_create_repository(self):
         try:
            w = self.connect.snapshot.create_repository(repository=self.create_repository["repository"],snapshot=self.create_repository["snapshot"],body=self.create_repository["body"],master_timeout=self.create_repository["master_timeout"])
         except Exception as e:
            print(e)
         return w
...
#,repository=get_snapshot["repository"],snapshot=get_snapshot["snapshot"],ignore_unavailable=get_snapshot["ignore_unavailable"],master_timeout=get_snapshot["master_timeout"],verbose=get_snapshot["verbose"]
    def get_snapshot(self):
        try:
            w = self.connect.snapshot.get_snapshot(repository=self.get_snapshot["repository"],snapshot=self.get_snapshot["snapshot"],ignore_unavailable=self.get_snapshot["ignore_unavailable"],master_timeout=self.get_snapshot["master_timeout"],verbose=self.get_snapshot["verbose"])
        except Exception as e:
            print(e)
        return w
...
#,repository=create_snapshot["repository"], snapshot=create_snapshot["snapshot"], body=create_snapshot["body"],master_timeout=create_snapshot["master_timeout"],wait_for_completion=create_snapshot["wait_for_completion"]
    def create_snapshot(self):
        try:
            snapshotcreate = self.connect.snapshot.create(repository=self.create_snapshot["repository"], snapshot=self.create_snapshot["snapshot"], body=self.create_snapshot["body"],master_timeout=self.create_snapshot["master_timeout"],wait_for_completion=self.create_snapshot["wait_for_completion"])
        except Exception as e:
            print(e)
...
#,snapshot=snapshot_delete["snapshot"],repository=snapshot_delete["repository"],master_timeout=snapshot_delete["master_timeout"]
    def snapshot_delete(self):
        try:
            self.connect.snapshot.delete(repository=self.snapshot_delete["repository"], snapshot=self.snapshot_delete["snapshot"],master_timeout=self.snapshot_delete["master_timeout"])
        except Exception as e:
            print(e)
...
#,repository=snapshot_restore["repository"], snapshot=snapshot_restore["snapshot"],body=snapshot_restore["body"],master_timeout=snapshot_restore["master_timeout"],wait_for_completion=snapshot_restore["wait_for_completion"]
    def snapshot_restore(self):
        try:
           restore = self.connect.snapshot.restore(repository=self.snapshot_restore["repository"], snapshot=self.snapshot_restore["snapshot"],body=self.snapshot_restore["body"],master_timeout=self.snapshot_restore["master_timeout"],wait_for_completion=self.snapshot_restore["wait_for_completion"])
        except Exception as e:
            print(e)
        return restore
...
print(a.info())
print(a.get_repository["repository"])
print(a.snapshot_get_repository())
